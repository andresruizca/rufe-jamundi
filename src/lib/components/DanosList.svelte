<script lang="ts">
	import type { DanoItem } from '$lib/instEducativasAggregate';

	const CAP = 30;
	const CAP_CRITICAL_ONLY = 80;

	let { items }: { items: (DanoItem & { critical: boolean })[] } = $props();
...
{#if items.length === 0}
	<p class="empty">Sin daños registrados para este filtro.</p>
{:else}
	{#if criticalCount > 0}
	...
	{/if}
	<ul class="obs-list">
		{#each visible as item (item.sede + item.texto)}
			<li class:critical={item.critical}>
				<div class="tags">
					<span class="sede-name" title={item.establecimiento}>{item.sede}</span>
					<span
						class="badge"
						class:urbana={item.zona === 'Urbana'}
						class:rural={item.zona === 'Rural'}>{item.zona}</span
					>
					{#if item.critical}
	...
	{#if source.length > cap}
		<p class="more">
			...
			buscador para ver un grupo más específico.
		</p>
	{/if}
{/if}
